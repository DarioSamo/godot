...
namespace respv {
    ...
    bool Shader::inlineData(const void *pData, size_t pSize) {
        ...
        struct Function {
            ...
            Function(uint32_t resultId) : resultId(resultId) {
        ...
        struct FunctionCall {
            ...
            FunctionCall(uint32_t wordIndex, uint32_t functionId) : wordIndex(wordIndex), functionId(functionId) {
                // Regular constructor.
            }
        };
        ...
        while (parseWordIndex < dataWordCount) {
            ...
            case SpvOpFunctionCall:
                ...
                functionCalls.emplace_back(parseWordIndex, dataWords[parseWordIndex + 3]);
            ...
            case SpvOpStore: {
                ...
                // Only accept the operation if it doesn't use memory operands.
                uint32_t pointerId = dataWords[parseWordIndex + 1];
                if (wordCount == 3) {
                    if (storeMap[pointerId] == UINT32_MAX) {
                        storeMap[pointerId] = dataWords[parseWordIndex + 2];
                    }
                    else if (storeMap[pointerId] < dataIdBound) {
                        storeMap[pointerId] = dataIdBound;
                    }
                }
                else if (storeMap[pointerId] != dataIdBound) {
                    storeMap[pointerId] = dataIdBound;
            ...
            // Check for any usage of the local variables that isn't just stores or loads. Do not propagate stored values if the variables are involved in any other operations.
            if ((opCode != SpvOpStore) && (opCode != SpvOpLoad)) {
                ...
                if (SpvHasOperands(opCode, operandWordStart, operandWordCount, operandWordStride, operandWordSkip, operandWordSkipString, true)) {
                    ...
                    for (uint32_t j = 0; j < operandWordCount; j++) {
                        if (checkOperandWordSkip(parseWordIndex, dataWords, j, operandWordSkip, operandWordSkipString, operandWordIndex)) {
                            continue;
                        }
                        ...
                        uint32_t shaderWordIndex = parseWordIndex + operandWordIndex;
                        uint32_t operandId = dataWords[shaderWordIndex];
                        if (storeMap[operandId] != dataIdBound) {
        ...
        typedef std::vector<Function>::iterator FunctionIterator;
        FunctionIterator entryFunctionIt = std::lower_bound(functions.begin(), functions.end(), entryPointFunctionId);
        if (entryFunctionIt == functions.end()) {
        ...
        struct FunctionItem {
            ...
            FunctionIterator rootFunction = {};
            uint32_t callIndex = 0;

            FunctionItem(FunctionIterator function, FunctionIterator rootFunction, uint32_t callIndex) : function(function), rootFunction(rootFunction), callIndex(callIndex) {
                // Regular constructor.
            }
        };

        std::vector<FunctionItem> functionStack;
        FunctionIterator startFunctionIt = functions.begin();
        while (startFunctionIt != functions.end()) {
        ...
        while (!functionStack.empty()) {
            ...
            if (functionItem.callIndex == functionItem.function->callCount) {
                ...
                FunctionIterator callFunctionIt = std::lower_bound(functions.begin(), functions.end(), callFunctionId);
                if (callFunctionIt == functions.end()) {
        ...
        while (!callStack.empty()) {
            ...
            case SpvOpFunction: {
                ...
                FunctionIterator functionIt = std::lower_bound(functions.begin(), functions.end(), functionId);
            ...
            case SpvOpVariable:
                if ((callStack.back().functionId < UINT32_MAX) && !callStack.back().functionInlined) {
                    ...
                    if (dstInlinedVariableWordIndex == UINT32_MAX) {
                        FunctionIterator functionIt = std::lower_bound(functions.begin(), functions.end(), callStack.back().functionId);
                        if (functionIt == functions.end()) {
            ...
            if (copyWords) {
                ...
                for (uint32_t i = 0; i < wordCount; i++) {
                    inlinedSpirvWords[copyWordIndex + i] = dataWords[callWordIndex + i];
                }
                ...
                if (SpvHasOperands(opCode, operandWordStart, operandWordCount, operandWordStride, operandWordSkip, operandWordSkipString, true)) {
                    ...
                    for (uint32_t j = 0; j < operandWordCount; j++) {
                        ...
                        if (loadMap[operandId] < dataIdBound) {
                            inlinedSpirvWords[shaderWordIndex] = loadMap[operandId];
                        }

                        callStack.back().remapsPending.emplace_back(shaderWordIndex);
                        operandWordIndex += operandWordStride;
                    }
                }
    ...
    bool Shader::parseData(const void *pData, size_t pSize) {
        ...
        uint32_t blockInstructionIndex = 0;
        ...
        while (wordIndex < dataWordCount) {
            ...
            case SpvOpPhi:
                ...
                break;
            case SpvOpLoopMerge:
                loopHeaders.emplace_back(uint32_t(instructions.size()), blockInstructionIndex);
                break;
            case SpvOpLabel:
                blockInstructionIndex = uint32_t(instructions.size());
                break;
            default:
                break;
            }
    ...
    bool Shader::sort(const void *pData, size_t pSize) {
        ...
        for (InstructionSort &instructionSort : instructionSortVector) {
            ...
            if (opCode == SpvOpVariable) {
                variableOrder.emplace_back(instructionSort.instructionIndex);
            }
        }
    ...
    static void optimizerReduceResultDegrees(OptimizerContext &rContext, std::vector<uint32_t> &rResultStack) {
        ...
        while (!rResultStack.empty()) {
            ...
            if ((rContext.instructionOutDegrees[instructionIndex] == 0) && (opCode != SpvOpFunctionCall)) {
                uint32_t wordCount = (optimizedWords[wordIndex] >> 16U) & 0xFFFFU;
        ...
        for (int32_t i = orderCount - 1; i >= 0; i--) {
            ...
            while (!accessStack.empty()) {
                ...
                if (rContext.instructionOutDegrees[accessInstructionIndex] > 0) {
                    uint32_t listIndex = accessInstruction.adjacentListIndex;
                    while (listIndex != UINT32_MAX) {
                        uint32_t adjacentInstructionIndex = rContext.shader.listNodes[listIndex].instructionIndex;
                        uint32_t adjacentWordIndex = rContext.shader.instructions[adjacentInstructionIndex].wordIndex;
                        listIndex = rContext.shader.listNodes[listIndex].nextListIndex;
            ...
            // and remap the results of the adjacent instructions.
            if (!fullLoadInstructionIndices.empty() && (storeInstructionIndices.size() == 1) && storeIsFull) {
                ...
                if (optimizedWords[storeWordIndex] != UINT32_MAX) {
                    uint32_t storeResultId = optimizedWords[storeWordIndex + 2];
                    ...
                    for (uint32_t loadInstructionIndex : fullLoadInstructionIndices) {
                        ...
                        uint32_t listIndex = loadInstruction.adjacentListIndex;
                        while (listIndex != UINT32_MAX) {
                            uint32_t adjacentInstructionIndex = rContext.shader.listNodes[listIndex].instructionIndex;
                            uint32_t adjacentWordIndex = rContext.shader.instructions[adjacentInstructionIndex].wordIndex;
                            if (optimizedWords[adjacentWordIndex] != UINT32_MAX) {
            ...
            if (fullLoadInstructionIndices.empty() && partialLoadInstructionIndices.empty()) {
