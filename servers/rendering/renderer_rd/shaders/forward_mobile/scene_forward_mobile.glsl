...
hvec4 fog_process(vec3 vertex) {
	...
	if (sc_use_fog_aerial_perspective()) {
		...
		vec3 sky_sample_a = texture(sampler2DArray(radiance_octmap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3(vec3_to_oct(cube_view), lod)).rgb;
		vec3 sky_sample_b = texture(sampler2DArray(radiance_octmap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3(vec3_to_oct(cube_view), lod + 1)).rgb;
		sky_fog_color = mix(sky_sample_a, sky_sample_b, blend);
#else
		sky_fog_color = textureLod(sampler2D(radiance_octmap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3_to_oct(cube_view), mip_level * MAX_ROUGHNESS_LOD).rgb;
...
// If alpha scissor is used, we must further the edge threshold, otherwise we won't get any edge feather
	...
	if (sc_scene_use_reflection_cubemap()) {
		...
		hvec3 indirect_sample_a = hvec3(texture(sampler2DArray(radiance_octmap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3(vec3_to_oct(ref_vec), float(lod))).rgb);
		hvec3 indirect_sample_b = hvec3(texture(sampler2DArray(radiance_octmap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3(vec3_to_oct(ref_vec), float(lod) + 1.0)).rgb);
		...
		indirect_specular_light = hvec3(textureLod(sampler2D(radiance_octmap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3_to_oct(ref_vec), lod).rgb);
#endif //USE_RADIANCE_OCTMAP_ARRAY
...
#ifdef USE_RADIANCE_OCTMAP_ARRAY
			hvec3 octmap_ambient = hvec3(texture(sampler2DArray(radiance_octmap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3(vec3_to_oct(ambient_dir), MAX_ROUGHNESS_LOD)).rgb);
#else
			hvec3 octmap_ambient = hvec3(textureLod(sampler2D(radiance_octmap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3_to_oct(ambient_dir), MAX_ROUGHNESS_LOD).rgb);
#endif //USE_RADIANCE_OCTMAP_ARRAY
	...
	if (sc_scene_use_reflection_cubemap()) {
		...
		hvec3 clearcoat_sample_a = hvec3(texture(sampler2DArray(radiance_octmap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3(vec3_to_oct(ref_vec), lod)).rgb);
		hvec3 clearcoat_sample_b = hvec3(texture(sampler2DArray(radiance_octmap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3(vec3_to_oct(ref_vec), lod + 1)).rgb);
		hvec3 clearcoat_light = mix(clearcoat_sample_a, clearcoat_sample_b, blend);
#else
		hvec3 clearcoat_light = hvec3(textureLod(sampler2D(radiance_octmap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3_to_oct(ref_vec), roughness_lod).rgb);
#endif //USE_RADIANCE_OCTMAP_ARRAY
